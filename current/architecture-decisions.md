...
1. `config/local.json`
2. 없으면 환경변수 `PHONE_CLAW_STORAGE_DIR`
...
즉, 우리는 MISO를 대체하지 않는다. Phone-Claw는 **Voice input -> structured handoff payload**를 만드는 입력/정리 레이어이고, MISO에는 이 payload를 받아 실행할 inbound workflow 기능을 제안한다.
...
> Phone-Claw는 회의와 통화 같은 일상의 음성을 로컬에서 전사하고, EXAONE으로 에이전트가 읽기 쉬운 업무 입력으로 바꿉니다.
